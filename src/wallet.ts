// Adapted from midnightntwrk/example-hello-world (Apache-2.0).
import {
  type CoinPublicKey,
  DustSecretKey,
  type EncPublicKey,
...
// `requireDust=false` skips the dust-stream gate: a fresh wallet with no NIGHT
// registered for dust generation may never report dust sync as strictly
// complete, which would hang the wait forever.
    ...
    wallet.state().pipe(
      ...
      Rx.filter(
        (state: FacadeState) =>
          ...
          (!requireDust || isProgressStrictlyComplete(state.dust.state.progress)) &&
